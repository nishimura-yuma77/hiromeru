...
4. **施策の直接編集は、SC-05で行える。削除の画面は作らない。** Agentに相談して変更する場合はSC-02で提案を受けて承認し、人が自分で書き換える場合はSC-05の編集フォームで保存する。どちらも施策のupsert（`API_DESIGN.md`の4.1）を使い、上書きには画面を表示した（または提案を受けた）時点の`updated_at`を使う。公開済みの投稿は、変更も削除もできない
...
| SC-05 | 施策詳細 | `/campaigns/{campaign_id}` | 施策の内容と、関連する投稿・記憶、投稿ごとの初週PV数を確認する。施策を直接編集する | `GET /campaigns/{campaign_id}`、`POST /agent-sessions/{id}/campaigns`（施策の編集）、`GET /agent-sessions`、`POST /agent-sessions`（保存先Sessionの選択） |
...
| SC-02 | `app/chat/`（`layout.tsx`に会話一覧、`page.tsx`が一覧）、`app/chat/new/`、`app/chat/[session_id]/` | `agent` | Server Component（会話一覧、履歴の取得） | 新しい会話の作成、メッセージ送信（SSEの受信）、Turnのポーリング、フォームの編集、承認 |
| SC-04、SC-05 | `app/campaigns/`、`app/campaigns/[campaign_id]/` | `campaigns` | Server Component | 検索、絞り込み（URLを更新する）、施策の編集（SC-05） |
...
- 業務データの読取は、Server Componentで行う（規約11.1）。Clientの再取得は、Turnの進捗の受信（SSE）と、切断時のTurnのポーリング、施策の選択の検索（6.11）だけとする（規約11.3。Turnは応答が最大200秒かかり、進捗を逐次表示し、応答を受け取れない場合に状態を確認する必要があるため。施策の選択は、入力に応じた候補の表示が必要なため）
...
- 会話一覧: `/chat`の本体であり、`/chat/new`と`/chat/{session_id}`では画面の左に表示する（`GET /agent-sessions`）。最終更新の新しい順で、20件ずつ読み込む（`cursor`）。表示は、会話のタイトルと最終更新日時とする
...
- 会話のタイトルは、最初のメッセージの先頭部分から自動で付く。タイトルがない会話（SC-05・SC-09の保存先として作成されたが、メッセージがまだない会話。6.13）は、「無題の会話」と表示する。アーカイブと削除の操作は設けない
...
**編集**
- 「編集」で、タイトル、ターゲット像、実施背景、施策目的、施策内容の5項目を、フォームとして編集できる。Agentは経由しない。フォームは、SC-02の施策フォームと同じ項目とする
- 保存は、施策のupsert（`API_DESIGN.md`の4.1）の上書きで行う。`id`に表示中の施策のID、`expected_updated_at`にSC-05を取得した時点の`updated_at`を、加工せずに設定する。5項目すべてを送る（部分更新はしない）
- 保存の前に、確認ダイアログ（「既存の内容を置き換えます」）を表示する。確認の操作ごとにUUIDを生成して`Idempotency-Key`に設定し、通信の再試行では同じ値を使う
- 4.1はSession単位のAPIのため、保存先の親Sessionは、最終更新が最新の親Sessionとする。決め方と、保存の副作用は、6.13に従う
- 保存に成功したら、編集フォームを閉じ、最新の施策を再取得して表示する（`router.refresh()`）
- `409 CAMPAIGN_CONFLICT`（表示後に、別のSessionまたはマーケターが更新した）の場合は、入力した内容をフォームに残したまま、「他の操作で更新されました」と表示する。「最新の内容を読み込む」で最新の施策を再取得し、入力した内容と見比べて、もう一度保存できるようにする。同じ`Idempotency-Key`では保存済みのエラーが返るため、保存し直すときは新しいキーを使う
...
- 忘却は、親Sessionの配下のAPIで実行する（`API_DESIGN.md`の7.1）。この画面には会話がないため、保存先は最終更新が最新の親Sessionとする。決め方と、保存の副作用は、6.13に従う。忘却の結果は、その会話の履歴に承認の結果として残る（記憶の内容は残らない）
...
### 6.1 グローバルナビ
ログイン後の全画面に表示する。項目は、チャット（会話一覧を含む）、施策、投稿、計測結果、記憶、ログアウトとする。ログイン中のメールアドレスを表示する。
...
| --- | --- | --- | --- |
| SC-02 | `/chat`、`/chat/new`、`/chat/{session_id}` | `/chat`だけ`cursor` | `/chat`は会話一覧（次のページを`cursor`で指定する）。`/chat/new`は新しい会話。会話はPathで指定する |
...
- `/chat/new`と`/chat/{session_id}`の左に並べる会話一覧は、最新の1ページ（20件）だけを表示する。それ以前の会話は、`/chat?cursor=...`の一覧で開く
...
### 6.13 保存先Sessionの決め方（SC-05・SC-09）
施策の直接編集（SC-05。`API_DESIGN.md`の4.1）と記憶の忘却（SC-09。同7.1）は、Session単位のAPIのため、操作の履歴を残す親Sessionが必要である。この2つの画面には会話がないため、**最終更新が最新の親Session**を保存先とする。SC-02の承認は、開いている会話（`/chat/{session_id}`）が保存先であり、本節の対象外とする。

**保存先の決め方**
1. 画面の表示時、`app/`のServer Componentが、`agent`Featureの公開関数で`GET /agent-sessions?limit=1`（5.5。子Sessionとアーカイブ済みを除く、`updated_at`が最新）を取得し、`targetSession`（`session_id`、または会話がなければ`null`）として、SC-05・SC-09のコンポーネントへpropsで渡す。`campaigns`・`memories`は`agent`を参照しない（規約4.4）
2. 確認ダイアログで確定した時点で、保存先を固定する。`targetSession`があればそのSession、`null`なら`POST /agent-sessions`（5.2）で作成したSessionとする
3. 保存先のSessionで、4.1または7.1を、確認の操作ごとに生成した`Idempotency-Key`で呼び出す
4. 同じ確認操作の再試行では、同じ`Idempotency-Key`と同じ保存先Sessionを使う。キーはSessionと組で照合されるため、再試行で保存先が変わると`409 IDEMPOTENCY_KEY_REUSED`になる。Sessionを作成した後にAPIが失敗した場合も、作成したSessionを保持し、再試行で作り直さない（空の会話を増やさない）。保存の処理中は、確定のボタンを無効にする

**保存先が使えない場合**
- `404 AGENT_SESSION_NOT_FOUND`（保存先がアーカイブされたなど。MVPにはアーカイブ操作がないため、通常は発生しない）は、操作が実行されていない。画面を再読み込み（`router.refresh()`）して保存先を取り直し、もう一度確認の操作をするよう促す。この再確認では、新しい`Idempotency-Key`を使う
- `409 TURN_IN_PROGRESS`は発生しない。保存先で会話のTurnが実行中でも、承認API・忘却APIのAPI実行Turnは、同時実行に数えないため、そのまま保存できる（2.10）

**確認ダイアログの表示**
- 確認ダイアログに、保存先の会話のタイトルを「この操作は会話『{タイトル}』の履歴に記録されます」と表示する（会話がなくSessionを新しく作る場合は「新しい会話に記録されます」）。保存先は、この画面では選び替えられない

**保存の結果（許容する副作用）**
- 保存先Sessionの`updated_at`が更新され、会話一覧の先頭に移る（5.5）
- 保存先の履歴に、`approval_action`と`api_result`が残る（SC-02で表示される。施策の編集では上書きした内容、記憶の忘却では忘却した事実だけで、記憶の内容は残らない）
- 保存先が無関係な会話であっても、Agentは次のTurnで`api_result`をContextに読み込み、変更を把握できる。会話のタイトルと内容は、変わらない
- 新しく作成したSessionは、`title`が`null`のまま残る（`API_DESIGN.md`の5.2）。会話一覧では「無題の会話」と表示し、その会話で最初のメッセージを送信すると、タイトルが付く（5.3）

**採用しなかった案**
- 専用の「手動操作」会話を作る案: 会話一覧に、利用者が作っていない会話が現れるため
- Sessionに紐づけない専用のAPIにする案: 承認監査、冪等性、API実行Turnの仕組みを別に作る必要があるため

## 7. 参照用のAPI

画面が使用する参照用のAPI（`API_DESIGN.md`の6章）と、記憶の忘却API（同7章）、SC-05の施策の編集で使う施策のupsert（同4.1）を示す。
...
| `POST /agent-sessions/{id}/campaigns` | SC-05 | 施策の直接編集（上書き。`Idempotency-Key`と`expected_updated_at`必須） | 4.1 |
...
- 記憶の忘却と、SC-05の施策の編集は、状態変更APIとして、CSRF対策と確認ダイアログを必須とする。UIは、確認の操作ごとにUUIDを生成して`Idempotency-Key`に設定し、再送では同じ値を使う
...
| 項目 | 内容 |
| --- | --- |
| SC-05の編集フォームの置き場所 | 施策フォームと投稿フォーム（チャット内の提案フォーム）は、`agent`Featureに置くと決定済み。SC-05の編集フォームは`campaigns`Featureに置くため、`agent`のフォームは参照できない（規約4.4）。項目の入力部品と、項目の定義（検証Schema）を、`shared`へ置くか、重複を許容するかを決める |
| 記憶の忘却・施策の編集の保存先Session | 最終更新が最新の親Sessionを保存先にする案Aで、6.13に具体化した（決め方、再試行、保存先が使えない場合、確認ダイアログの表示、副作用）。無関係な会話の履歴に記録され、会話一覧の先頭に移る点を許容するか、レビューで確認する |
| 会話一覧の表示件数 | `/chat/new`と`/chat/{session_id}`の左の一覧は最新20件だけを表示する。それ以前の会話を、一覧の中で続けて読み込めるようにするか（Client側の再取得は規約11.3の例外になる） |
