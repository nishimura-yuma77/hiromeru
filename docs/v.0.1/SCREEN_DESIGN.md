...
| SC-09 | 記憶一覧 | `/memories` | 長期記憶を意味検索または施策で絞り込み、不要な記憶を忘却する | `GET /memories`、`DELETE /memories/{memory_id}` |
...
| SC-09 | `app/(authenticated)/memories/` | `memories` | Server Component | 検索、施策による絞り込み、忘却 |
...
- `/chat/new?campaign_id={id}&intent=create_post`では「施策ID {id}のX投稿案を作りたいです」、`intent=revise_campaign`では「施策ID {id}の内容を変更したいです」をComposerへ設定してFocusを移す。相談例と同様に自動送信しない
- 引継ぎの`campaign_id`は正の整数、`intent`は`create_post`または`revise_campaign`だけを許可する。一方だけが不正な場合も両方を無視し、空のComposerを表示する。SC-02はParamsから施策の存在確認を行わず、Agentが送信後の通常フローで会社所有権を検証する
...
- 記憶が0件の場合は「この施策に関連する記憶はありません」を表示する。この画面から追加・忘却は行わない
...
- `campaign_id`がある場合は、指定した施策に関連付けられた記憶だけを表示し、「施策ID {id}に関連する記憶」と「施策の絞り込みを解除」を表示する。SC-05の「関連する記憶をすべて見る」からこの状態で開く
- `campaign_id`で絞り込んだ結果が0件の場合は「この施策に関連する記憶はありません」と「施策の絞り込みを解除」を表示する。会社全体の記憶が0件であるようには見せない
- `campaign_id`と`query`は併用できる。`404 CAMPAIGN_NOT_FOUND`では他社IDと存在しないIDを区別せず、「施策が見つかりません」と「施策の絞り込みを解除」を表示する
...
| --- | --- | --- | --- |
| SC-02 | `/chat`、`/chat/new`、`/chat/{session_id}` | `/chat/new`だけ`campaign_id`、`intent` | `/chat`は会話一覧。`/chat/new`は新しい会話。会話はPathで指定する。SC-05からの引継ぎParamsはComposerの初期文だけに使う。会話一覧は無限スクロール（6.13）のため、`cursor`はURLに持たない |
