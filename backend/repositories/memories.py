...
from models import AgentMemory, Campaign, MemoryCampaign, MemoryPost, Post
...
class MemoryRepository:
    ...
    async def list_recent(
        self, company_id: int, *, limit: int, after_id: int | None
    ) -> list[MemoryRow]:
    ...
    async def search(self, company_id: int, vector: list[float], limit: int) -> list[MemoryRow]:
        """コサイン類似度の高い順に記憶を返す。"""
        distance = cosine_distance(AgentMemory.embedding, vector)
        stmt = (
            select(AgentMemory.id, AgentMemory.content, (1 - distance).label("similarity"))
            .where(AgentMemory.company_id == company_id)
            .order_by(distance, AgentMemory.id.desc())
            .limit(limit)
        )
    ...
    async def relations(self, company_id: int, memory_ids: list[int]) -> MemoryRelations:
        ...
        post_stmt = (
            ...
            .where(MemoryPost.memory_id.in_(memory_ids), Post.company_id == company_id)
            .order_by(Post.id)
        )
